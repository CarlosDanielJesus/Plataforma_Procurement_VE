...
if seleccion == "Inicio":
    ...
    if st.session_state['rol_activo'] == 'Proveedor':
        ...
        st.write(f"Gestionando catálogo para: **{st.session_state['usuario_activo']}**")
        
        # Selección de modo de operación
        modo_operacion = st.radio("¿Qué deseas hacer?", ["Cargar Inventario Nuevo", "Actualizar Inventario Existente"])
        ...
        if archivo_subido is not None:
            ...
            if df_nuevo is not None:
                ...
                df_nuevo['PROVEEDOR'] = st.session_state['usuario_activo']
                ...
                # --- BOTÓN DE ACCIÓN ---
                label_boton = "Guardar en Base de Datos" if modo_operacion == "Cargar Inventario Nuevo" else "Actualizar Inventario Existente"
                
                if st.button(label_boton):
                    # Aquí pasamos un parámetro extra a tu función de base de datos
                    # que indique si es modo 'reemplazo' o 'actualización'
                    es_actualizacion = (modo_operacion == "Actualizar Inventario Existente")
                    
                    # NOTA: Debes ajustar tu función 'guardar_inventario_en_bd' para que 
                    # acepte el booleano 'es_actualizacion' y gestione el UPSERT en SQL
                    guardar_inventario_en_bd(df_nuevo, st.session_state['id_usuario'], actualizar=es_actualizacion)
                    
                    if es_actualizacion:
                        st.success("¡Catálogo actualizado con éxito! Los precios y stocks antiguos han sido sobrescritos.")
                    else:
                        st.success("¡Nuevo inventario cargado exitosamente!")
            else:
