...
def obtener_catalogo_completo():
    ...
    conn = conectar_db()
    try:
        # AÑADIDO: 'AS SKU', 'AS MATERIAL', etc., para que Pandas reciba las columnas en MAYÚSCULAS 
        # y coincidan exactamente con lo que pide el app.py
        query = """
            SELECT 
                i.sku AS SKU, 
        ...
        # Aseguramos que el dataframe vacío también tenga las columnas en mayúsculas
...
def guardar_inventario_en_bd(df_inventario, proveedor_id, actualizar=False):
    ...
    """
    # CORRECCIÓN: Usar la función conectar_db() para apuntar a 'plataforma.db'
    conn = conectar_db()
    cursor = conn.cursor()
